...
export default class BlackHole extends Model {
    ...
    getQualitySettings() {
        ...
        // Determine quality tier - Be more conservative to ensure visual quality
        let qualityTier
        if (isMobileDevice && (cores <= 4 || memory <= 3)) {
            qualityTier = 'low' // Mobile with limited resources
        } else if (isMobileDevice || cores <= 4) {
            qualityTier = 'medium' // Mobile or low-mid range desktop
        } else if (cores <= 8) {
            qualityTier = 'high' // Mid-high range desktop
        } else {
            qualityTier = 'ultra' // High-end desktop
        }
        
        const settings = {
            low: { iterations: 64, stepSize: 0.009, noiseFactor: 0.008 },      // Minimum for visual quality
            medium: { iterations: 80, stepSize: 0.008, noiseFactor: 0.009 },   // Balanced
            high: { iterations: 112, stepSize: 0.0071, noiseFactor: 0.01 },    // High quality
            ultra: { iterations: 128, stepSize: 0.006, noiseFactor: 0.012 }    // Maximum
        }
        
        console.log(`🕳️ Black Hole quality: ${qualityTier} (${settings[qualityTier].iterations} iterations) | Cores: ${cores} | Memory: ${memory}GB | Mobile: ${isMobileDevice}`)
        return settings[qualityTier]
    }

    uniforms = {
        iterations: uniform( float( this.getQualitySettings().iterations ) ),
        stepSize: uniform( float( this.getQualitySettings().stepSize ) ),
        noiseFactor: uniform( float( this.getQualitySettings().noiseFactor ) ),
    ...
    setModel() {
        // Adaptive geometry resolution based on device
        const isMobileDevice = isMobile.any()
        const cores = navigator.hardwareConcurrency || 4
        
        // Ensure minimum segments for proper rendering
        let segments
        if (isMobileDevice && cores <= 4) {
            segments = 16 // Minimum for visual quality
        } else if (isMobileDevice || cores <= 6) {
            segments = 20
        } else {
            segments = 24 // High quality
        }
